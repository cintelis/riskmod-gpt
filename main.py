...
st.set_page_config(page_title="RISKMOD GPT", 
                   page_icon="https://cintelis.io/wp-content/uploads/2023/10/favicon-72x72-1.png", 
                   layout="wide", 
                   initial_sidebar_state="expanded"
                   )
st.header("RISKMOD GPT")
...
with col1:
    ...
    sensitive_data = st.selectbox(
        ...
        options=["Restricted", "Confidential", "Internal", "Public", "Unclassified", "None"],
        key="sensitive_data"
        )
...
st.sidebar.header("How to use RISKMOD GPT")
...
with st.sidebar:
    st.markdown("Welcome to RISKMOD GPT, an AI-powered tool designed to help teams produce better threat models for their applications.")
    st.markdown("Threat modelling is a key activity in the software development lifecycle, but is often overlooked or poorly executed. RISKMOD GPT aims to help teams produce more comprehensive threat models by leveraging the power of OpenAI's GPT models to generate a threat list, attack tree and/or mitigating controls for an application based on the details provided.")
    st.markdown("Created by [Nick Forshteyn](https://github.com/cintelis/).")
    # Add "Star on GitHub" link to the sidebar
    st.sidebar.markdown("⭐ Star on GitHub: [![Star on GitHub](https://img.shields.io/github/stars/cintelis/riskmod-gpt?style=social)](https://github.com/mrwadams/stride-gpt)")
    st.markdown("""---""")
...
with st.sidebar:
    st.markdown("Below is an example application description that you can use to test RISKMOD GPT:")
...
with st.sidebar:
    ...
    No, RISKMOD GPT does not store your application description or other details. All entered data is deleted after you close the browser tab.
    """)
    st.markdown("""
    ...
    No, the threat models are not 100% accurate. RISKMOD GPT uses GPT Large Language Models (LLMs) to generate its output. The GPT models are powerful, but they sometimes makes mistakes and are prone to 'hallucinations' (generating irrelevant or inaccurate content). Please use the output only as a starting point for identifying and addressing potential security risks in your applications.
    """)
    st.markdown("""
